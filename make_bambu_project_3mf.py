#!/usr/bin/env python3
"""Wrap an OpenSCAD 3MF mesh in a Bambu Studio project 3MF template."""
...
def create_project(
    ...
    source_files = read_zip(source)

    source_root = ET.fromstring(source_files[ROOT_MODEL])
    source_object = source_root.find(f".//{{{CORE_NS}}}object")
    if source_object is None:
        raise ValueError(f"No object found in {source}")
    source_mesh = source_object.find(f"{{{CORE_NS}}}mesh")
    if source_mesh is None:
        raise ValueError(f"No mesh found in {source}")

    normalized_mesh, stats = normalize_mesh(source_mesh)
...
def main() -> int:
    ...
    parser.add_argument("--template", default="templates/bambu/gridfinity_bin_template.3mf", type=Path)
    ...
    stats = create_project(args.template, args.source, args.output, args.front_margin, args.plate_width, args.rotate_z)
